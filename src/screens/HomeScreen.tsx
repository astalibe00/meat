import { Search, ChevronRight, BadgeCheck, Truck, Leaf, Clock, Sparkles, Flame } from "lucide-react";
import { useApp } from "@/store/useApp";
import { CATEGORIES, getProductsByTag, PRODUCTS, FREE_SHIPPING_THRESHOLD } from "@/data/products";
...
const TRUST_PILLS = [
  { Icon: BadgeCheck, label: "HMC Certified" },
  ...
  return (
    <div className="animate-screen-in pb-6">
      ...
      <div className="px-5 pt-4 pb-3">
        <p className="text-xs text-muted-foreground font-medium">Assalamu Alaykum 👋</p>
      ...
      <div className="px-5">
        ...
        >
          ...
          <span className="text-sm text-muted-foreground flex-1">Search beef, lamb, chicken…</span>
      ...
      <div className="mt-5 overflow-x-auto no-scrollbar">
        <div className="flex gap-3 px-5 pb-1">
          {banners.map((b) => (
            <div
              key={b.id}
              className="shrink-0 w-[296px] h-[160px] rounded-3xl overflow-hidden relative shadow-card"
            >
              <img
                src={b.img}
                alt=""
              ...
              <div className="absolute inset-0 p-5 flex flex-col justify-end text-primary-foreground">
                ...
                <span className="mt-2.5 inline-flex items-center text-[11px] font-bold w-fit">
                  {b.cta} <ChevronRight className="w-3.5 h-3.5 -mr-1" strokeWidth={2.5} />
                </span>
              </div>
            </div>
          ))}
      ...
      {/* Halal certification block — editorial */}
      <div className="mt-7 mx-5 rounded-3xl bg-foreground text-background p-6 relative overflow-hidden shadow-elevated">
        ...
        <div className="relative">
          <span className="chip bg-primary text-primary-foreground mb-3">
            <BadgeCheck className="w-3 h-3" strokeWidth={3} /> Certified
          </span>
          <h3 className="font-serif text-[22px] leading-tight font-semibold mb-1.5">
            100% Halal,<br />no compromises.
          </h3>
          <p className="text-xs text-background/70 leading-relaxed max-w-[280px]">
            Every cut is HMC-certified, ethically sourced, and hand-slaughtered following Islamic tradition.
          </p>
          <div className="mt-4 flex items-center gap-3 text-[11px] font-semibold">
            <span className="flex items-center gap-1.5">
              <BadgeCheck className="w-3.5 h-3.5 text-primary" strokeWidth={2.5} /> HMC
            </span>
            <span className="w-px h-3 bg-background/20" />
            <span className="flex items-center gap-1.5">
              <Leaf className="w-3.5 h-3.5 text-primary" strokeWidth={2.5} /> Ethical
            </span>
            <span className="w-px h-3 bg-background/20" />
            <span className="flex items-center gap-1.5">
              <Flame className="w-3.5 h-3.5 text-primary" strokeWidth={2.5} /> Hand-cut
            </span>
          </div>
        </div>
      </div>
